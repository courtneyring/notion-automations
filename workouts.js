const fs = require('fs');
const { buildProperties2, createPage } = require('./notion');
...
const parent = {
    type: 'database_id',
    database_id: 'a93d2d2e68d841148d9f6e4b5f6ece81'
}
...
const requiredProps = [
    ...
    { prop: 'activeEnergyBurned', field: 'Active Energy (kcal)', type: 'numWithUnits' },
...
const formatFields = (runs) => {
    let formatted = runs.map((run) => {
        ...
        obj['Date'] = { value: { start: formatDate(obj.start.value), end: formatDate(obj.end.value) }, type: 'date' }
        obj['id'].type='title'
        delete obj.start;
        delete obj.end;
        return obj;
